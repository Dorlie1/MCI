...
{roles_text}"""
    ...
    def _create_pdf(self, form_data):
        ...
        coord_labels = [
            ...
            ("Date de salut", form_data.get('date_du_salut', '')),
            ("Êtes-vous baptisé(e) par immersion ?", form_data.get('baptise', '')),
            ("Date de baptême", form_data.get('date_du_bapteme', '')),
        ...
        # Get the personality types from the form data (e.g., ['D/S', 'S/D'] or ['D', 'S'])
        personality_types = form_data.get('personality_type', [])

        # If you store full names, map them to codes
        trait_map = {
            'Dominance': 'D',
            'Influence': 'I',
            'Stabilité': 'S',
        ...
        # If types are like 'D/S', split and map
        disc_types = []
        for t in personality_types:
            if '/' in t:
                parts = [trait_map.get(x.strip(), x.strip()) for x in t.split('/')]
                # Try both orders
                disc_types.append(f"{parts[0]}/{parts[1]}")
                disc_types.append(f"{parts[1]}/{parts[0]}")
            else:
                disc_types.append(trait_map.get(t.strip(), t.strip()))

        # Remove duplicates, keep order
        seen = set()
        disc_types = [x for x in disc_types if not (x in seen or seen.add(x))]

        # Build the descriptions dict
        disc_descriptions = {t: DISC_MESSAGES.get(t, "") for t in disc_types if t in DISC_MESSAGES}
        form_data['disc_descriptions'] = disc_descriptions

        # DISC descriptions: you need to provide disc_descriptions as a dict {type: description}
        disc_table_data = [
        ...
        scores = form_data.get('personality_scores', {})
        egalite_message = ""
        if scores:
            max_score = max(scores.values())
            traits_max = [trait for trait, score in scores.items() if score == max_score]
            if len(traits_max) > 1:
                egalite_message = (
        ...
        don_codes = form_data.get('dons_grand') or form_data.get('dons') or []
        if isinstance(don_codes, str):
            don_codes = [don_codes]
        don_rows = self.db.get_don_descriptions(don_codes)
        don_descriptions = {row['nom']: row['description'] for row in don_rows}
        regions = [row['nom'] for row in self.db.get_regions()]
        don_roles_by_region = {}
        for don_code in don_codes:
            don_name = self.db.get_don_code_to_name().get(don_code, don_code)
            don_roles_by_region[don_name] = {}
            for region in regions:
                roles = [
                    row['nom']
                    for row in self.db.get_all_roles_with_don()
                    if row['don_code'] == don_code and row['region'] == region
                ]
                don_roles_by_region[don_name][region] = roles
        ...
        don_table = Table(don_table_data, colWidths=[1.2*inch, 3.5*inch] + [2*inch]*len(regions))
        ...
        story.append(Spacer(1, 10))
        dispo_data = [[
            Paragraph(f"<b>Temps disponible</b><br/>{form_data.get('disponibilite', '')}", styles['normal']),
    ...
    def send_email(self, form_data):
        ...
        genre = form_data.get('genre', '')
        civilite = "Monsieur" if genre == "Homme" else "Madame" if genre == "Femme" else "Monsieur/Madame"
        destinataire = f"{civilite} {prenom} {nom}"

        body = f"""
Bonjour {destinataire},

Merci d'avoir complété les tests de personnalité et de dons. Votre résultat est en pièce jointe.
Nous vous invitons à poser toutes vos questions durant le cours.

"""

        # Ajout de la consigne si résultats identiques
        scores = form_data['personality_scores']
        max_score = max(scores.values())
        traits_max = [trait for trait, score in scores.items() if score == max_score]
        if len(traits_max) > 1:
            body += (
                "Vous avez obtenu des résultats identiques pour votre test de personnalité. Nous vous invitons donc à :\n"
                "1.- Lire la définition de chaque trait de personnalité de votre résultat.\n"
                "2.- Envoyer un courriel à :\n"
                "   - Si Montréal : decouverte@mcigc.ca pour nous mentionner le trait de personnalité qui vous représente le mieux.\n"
                "   - Si St-Bruno : decouverters@mcigc.ca pour nous mentionner le trait de personnalité qui vous représente le mieux.\n\n"
            )
